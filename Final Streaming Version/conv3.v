...
module conv3 #(
    ...
    // ---- split huge weight array into two halves ----
    localparam int FILTER_SIZE = IN_CH * K_ELEMS;
    localparam int OUT_CH_HALF = OUT_CH / 2;

    // filters 0..63
    reg signed [N-1:0] weight_mem0 [0:OUT_CH_HALF-1][0:FILTER_SIZE-1];
    // filters 64..127
    reg signed [N-1:0] weight_mem1 [0:OUT_CH-OUT_CH_HALF-1][0:FILTER_SIZE-1];
    ...
    always @(posedge clk) begin
        if (weight_wr_en) begin
            ...
            filter_id = weight_wr_addr / FILTER_SIZE;
            coeff_idx = weight_wr_addr % FILTER_SIZE;

            if (filter_id < OUT_CH_HALF)
                weight_mem0[filter_id][coeff_idx] <= weight_wr_data;
            else
                weight_mem1[filter_id-OUT_CH_HALF][coeff_idx] <= weight_wr_data;
        end
    ...
    // ---- window generation (same as before) ----
    ...
    assign all_windows_valid =
        (&{ window_valid_ch[0], window_valid_ch[1], window_valid_ch[2], window_valid_ch[3],
            window_valid_ch[4], window_valid_ch[5], window_valid_ch[6], window_valid_ch[7] }) ? 1'b1 : 1'b1;
    ...
    reg [31:0] out_pixel_counter;

    // helper: select weight from the right memory half
    reg signed [N-1:0] weight_val;
    always @(*) begin
        if (f_idx < OUT_CH_HALF)
            weight_val = weight_mem0[f_idx][ ch_idx*K_ELEMS + kidx ];
        else
            weight_val = weight_mem1[f_idx-OUT_CH_HALF][ ch_idx*K_ELEMS + kidx ];
    end
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            case (s)
                ...
                S_ACC_CHK: begin
                    prod <= $signed(ws[ch_idx][kidx]) * $signed(weight_val);
                    acc  <= acc + prod;
                ...
                S_STORE: begin
                    ...
                    acc_shifted = (acc >>> Q) +
                                  $signed({{(48-N){bias_mem[f_idx][N-1]}}, bias_mem[f_idx]});
